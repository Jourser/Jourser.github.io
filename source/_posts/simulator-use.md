...
#### 当需要指定测试某个模块功能时？ 
- 当项目中留有多个仿真文件时，可以设置**顶层仿真文件**。通常在一个仿真集中（`sim_1`），只会有一个文件被指定为顶层模块。而其他模块可能是子模块，或者是被顶层仿真文件实例化的模块，或者是其他独立的测试文件。
- **<font color="#dd0000">如何制定顶层仿真文件？</font>**
   1. 在设计源文件列表中找到您想仿真的测试平台文件。
   2. 右键点击该文件，然后选择“设置为顶层文件”（`Set as Top`）。
   3. 运行仿真。

![](./simulator-use/18.png)
