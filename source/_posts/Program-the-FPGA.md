...
<img src=./Program-the-FPGA/13.png width=80%/>
   ...
   - ![](Program-the-FPGA/31.png)
      1. `Configuration File Only` (仅配置文件)
            - 范围： 只对配置文件（.mcs 或 .bin 文件）所占据的存储区域进行操作。
            - 行为： 工具会计算文件的大小，然后只擦除、编程和校验 Flash 中对应的那些地址段。
            - 优点： 速度快。因为大多数 Flash 芯片的容量远大于配置文件，只操作必要部分可以节省大量时间。
            - 缺点： Flash 中超出文件范围的其他区域会保留旧数据。
      2. `Entire Configuration Memory Device` (整个配置存储设备)
            - 范围： 对整颗 Flash 芯片的所有物理地址进行操作。
            - 行为： 工具会对整个芯片执行全局擦除（Bulk Erase），然后写入配置文件，并对整个芯片进行校验。
            - 优点： 更彻底、更干净。确保 Flash 中没有残留的旧固件或垃圾数据，排除潜在干扰。
            - 缺点： 非常慢。对于大容量（如 1Gb 或更高）的 Flash，擦除和校验全片可能需要几分钟甚至更久。
  -  ![](Program-the-FPGA/32.png)
      1. `Erase` (擦除)
            - 功能： 在写入新数据之前，将 Flash 中存储的内容清空。
            - 原理： Flash 存储器的特性是只能“由 1 变为 0”，不能直接从 0 变回 1。因此，在编程前必须先执行擦 除动   - 作，将 选定区域的所有比特位全部置为 1（即全为 0xFF）。
            - 注意： 如果不勾选此项，且 Flash 原本不为空，编程很可能会失败。
      2. `Blank Check` (查空)
            - 功能： 检查芯片内部是否已经处于“空白”状态（即全为 1）。
            - 目的： 这是一个验证步骤，用来确认“Erase”动作是否彻底成功。如果查空失败，说明芯片里还有残留数  据，不能    - 进行    下一步编程。
            - 建议： 正常开发时通常不勾选，以节省时间。
      3. `Program` (编程/写入)
            - 功能： 将你的配置文件（.bit, .mcs, .bin 等）真正地写入到 Flash 芯片中。
            - 重要性： 这是核心步骤。它会按照文件的内容，把芯片中对应的比特位从 1 变成 0。
      4. `Verify` (校验)
            - 功能： 把刚刚写入 Flash 的数据重新读出来，与电脑里的源文件进行逐字节对比。
            - 目的： 确保写入过程没有任何差错（例如因为电压不稳或干扰导致的位错误）。
            - 代价： 会增加烧录时间，因为需要完整的读取过程。
      5. `Verify Checksum` (校验和验证)
            - 功能： 同样是验证，但它不进行逐字节对比，而是通过数学算法计算出一个校验和（Checksum）并进行比对。
            - 区别： 相比于上面的 Verify，它的速度通常更快，但严谨程度略低（虽然在绝大多数情况下已经足够可靠）。



**PS:基本上只需勾选**`Erase`**和**`Program`**，在烧录过程中意外断电，则在重新烧录时勾选**`Verify`
